...
export class PokemonService {
  ...
  static async searchPokemon(query: string): Promise<Pokemon | null> {
    try {
      ...
      // 1. 먼저 영어 이름이나 ID로 직접 검색
      try {
        return await this.getPokemon(normalizedQuery);
      } catch {
        // 영어 이름/ID로 찾을 수 없으면 계속
      }
      
      // 2. 한국어 이름으로 검색 (KOREAN_NAMES에서 영어 이름 찾기)
      const englishName = Object.entries(KOREAN_NAMES).find(
        ([englishName, koreanName]) => koreanName === query.trim()
      )?.[0];
      
      if (englishName) {
        return await this.getPokemon(englishName);
      }
      
      // 3. 부분 일치 검색 (한국어 이름에서)
      const partialMatch = Object.entries(KOREAN_NAMES).find(
        ([englishName, koreanName]) => 
          koreanName.includes(query.trim()) || query.trim().includes(koreanName)
      )?.[0];
      
      if (partialMatch) {
        return await this.getPokemon(partialMatch);
      }
